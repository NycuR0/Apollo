...
class Session{
	...
	private $mtuSize = 508; //(Max IP Header Size) â€” (UDP Header Size) = 576 â€” 60 â€” 8 = 508
	private $id = 0;
	private $splitID = 0;
		...
		if($this->state === self::STATE_CONNECTED or $this->state === self::STATE_CONNECTING_2){
			...
			if($packet instanceof OPEN_CONNECTION_REQUEST_1){
				...
				if($packet->serverPort === $this->sessionManager->getPort() or !$this->sessionManager->portChecking){
 					$this->mtuSize = min(abs($packet->mtuSize), 1432);  //MTU â€” (Max IP Header Size) â€” (UDP Header Size) = 1500 â€” 60 â€” 8 = 1432 
